...
var headers = {
  ...
  getStream = __name(function (_0) {
    return __async(
      this,
      arguments,
      function* ({ link: url, type: type, providerContext: providerContext }) {
        var _a, _b;
        const { axios: axios, cheerio: cheerio } = providerContext;
        try {
          const modGetEpisodeLinks = __name(function (_02) {
            return __async(
              this,
              arguments,
              function* ({ url: url2, providerContext: providerContext2 }) {
                var _a2;
                const { axios: axios2, cheerio: cheerio2 } = providerContext2;
                try {
                  url2.includes("url=") && (url2 = atob(url2.split("url=")[1]));
                  const html = (yield axios2.get(url2)).data;
                  let $ = cheerio2.load(html);
                  if (url2.includes("url=")) {
                    const newUrl =
                        null ==
                        (_a2 = $("meta[http-equiv='refresh']").attr("content"))
                          ? void 0
                          : _a2.split("url=")[1],
                      html2 = (yield axios2.get(newUrl || url2)).data;
                    $ = cheerio2.load(html2);
                  }
                  const episodeLinks = [];
                  return (
                    $("h3,h4").map((i, element) => {
                      const seriesTitle = $(element).text(),
                        episodesLink = $(element).find("a").attr("href");
                      episodesLink &&
                        "#" !== episodesLink &&
                        episodeLinks.push({
                    ...
                    $("a.maxbutton").map((i, element) => {
                      const seriesTitle = $(element).children("span").text(),
                        episodesLink = $(element).attr("href");
                      episodesLink &&
                        "#" !== episodesLink &&
                        episodeLinks.push({
                          title: seriesTitle.trim() || "No title found",
          ...
          const ddl =
              ...
              (_b =
                ...
                : _b[1]) || url,
            servers = [],
            driveLink = yield isDriveLink(ddl),
            driveHtml = (yield axios.get(driveLink, { headers: headers })).data,
            $drive = cheerio.load(driveHtml);
          try {
            const resumePath =
              $drive('a[href*="/zfile/"]').attr("href") ||
              $drive("a.btn-warning").attr("href") ||
              "";
            if (resumePath) {
              const resumePageUrl = new URL(resumePath, driveLink).href;
              const resumePageHtml = (
                yield axios.get(resumePageUrl, {
                  headers: { ...headers, Referer: driveLink },
                })
              ).data;
              const $resume = cheerio.load(resumePageHtml);
              $resume("a.btn-success[href]").each((i, element) => {
                const link = $resume(element).attr("href") || "";
                if (!/^https?:\/\//i.test(link)) return;
                servers.push({
                  server: i === 0 ? "Resume Cloud" : `Resume Cloud ${i + 1}`,
                  link,
                  type: "mkv",
                  headers: { Referer: resumePageUrl },
                });
              });
            }
          } catch (err) {
            console.log("Resume Cloud link not found", err);
          }
          try {
            ...
            ((newLink =
              newLinkRes.status >= 300 && newLinkRes.status < 400
                ...
                : newLinkRes.url && newLinkRes.url !== seed
                  ...
                  : newLinkRes.headers.get("location") || seed),
              console.log(
                "Gdrive-Instant-2 link",
                null == newLink ? void 0 : newLink.split("?url=")[1],
              ),
              servers.push({
  ...
  return __async(this, null, function* () {
    ...
    try {
      ...
      const formUrl = $("form").attr("action") || url.split("?")[0],
        ...
        link = (yield res2.text()).match(/setAttribute\("href",\s*"(.*?)"/)[1];
      console.log(link);
      const cookie = link.split("=")[1];
      return (
        console.log("cookie", cookie),
        yield axios.get(link, {
          headers: { Referer: formUrl, Cookie: `${cookie}=${wpHttp2}` },
        })
      );
    } catch (err) {
...
("use strict");
const __kandoProviderId = "topmovies";
