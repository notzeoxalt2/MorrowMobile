        ...
        if (processLink.includes("oxxfile"))
          try {
            const urlObj = new URL(processLink),
              id = processLink.split("/").filter(Boolean).pop(),
              apiUrl = `${urlObj.origin}/api/s/${id}/hubcloud`,
              res = yield fetch(apiUrl, {
                headers: commonHeaders,
                redirect: "follow",
                signal: signal,
              });
            res.url && res.url.includes("hubcloud") && (processLink = res.url);
          } catch (e) {
...
const __kandoProviderId = "1cinevood";
