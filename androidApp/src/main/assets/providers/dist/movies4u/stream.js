...
function getStream(_0) {
  return __async(
    ...
    function* ({
      ...
      try {
        const streamLinks = [];
        if ((console.log("dotlink", link), "movie" === type)) {
          let dotlinkRes, cookies;
          try {
          ...
          if (link.includes("fastdl.zip"))
            try {
              const reurlMatch = (yield axios.get(link, {
                headers: headers,
              })).data.match(/var reurl = "([^"]+)";/);
          ...
          try {
            ...
            if (null == (_b = filepressTokenRes.data) ? void 0 : _b.status) {
              const filepressToken =
                  null == (_c = filepressTokenRes.data) ? void 0 : _c.data,
              ...
              streamLinks.push({
                ...
                link:
                  null ==
                  (_e =
                    null == (_d = filepressStreamLink.data) ? void 0 : _d.data)
                    ? void 0
                    : _e[0],
                type: "mkv",
              });
...
("use strict");
const __kandoProviderId = "movies4u";
const __kandoLabelProvenance = "reviewed-host";
const __kandoLegacyGetStream = exports.getStream;
const __kandoResolveDirectFilePress = async (args) => {
  const requestedLink = String(args?.link || "").trim();
  if (
  ...
  const providerContext = args?.providerContext || {};
  ...
  try {
    const tokenResponse = await providerContext.axios.post(
      `${baseUrl}/api/file/downlaod/`,
      requestBody,
      { headers: apiHeaders, signal: args?.signal },
    );
    const token = tokenResponse?.data?.status ? tokenResponse.data.data : "";
    if (!token) return [];
    const streamResponse = await providerContext.axios.post(
...
exports.getStream = async (args) => {
  ...
  const filePressStreams = await __kandoResolveDirectFilePress({
    ...args,
    providerContext,
  });
  if (filePressStreams !== null) {
    return __kandoNormalizeStreamV2(filePressStreams, commonHeaders);
  }
