...
function gdflixExtractor(
  ...
  return __async(this, null, function* () {
    ...
    try {
      ...
      return (console.log("gdflix error: ", error), []);
    }
  });
}
function getWithWAF(url, axios, openWebView, headers, customHeaders) {
  return __async(this, null, function* () {
    ...
    const baseUrl = url.split("/").slice(0, 3).join("/"),
      mergedHeaders = __spreadProps(
        __spreadValues(__spreadValues({}, headers), customHeaders),
        { Referer: baseUrl },
      );
    try {
...
function extractKmhdLink(katlink, providerContext) {
  return __async(this, null, function* () {
    const {
      ...
      data = (yield getWithWAF(katlink, axios, openWebView, commonHeaders, {
        Cookie: "unlocked=true",
      })).data,
      hubDriveRes = data.match(/hubdrive_res:\s*"([^"]+)"/)[1];
    return (
      data.match(/hubdrive_res\s*:\s*{[^}]*?link\s*:\s*"([^"]+)"/)[1] +
      hubDriveRes
    );
  });
}
  ...
  return __async(
    ...
    function* ({
      ...
      try {
        ...
        if (link.includes("gdflix")) {
          try {
            const resumeDrive = link.replace("/file", "/zfile"),
              resumeDriveHtml = (yield getWithWAF(
                resumeDrive,
                axios,
                openWebView,
                commonHeaders,
          ...
          try {
            const driveres = yield getWithWAF(
                link,
                axios,
                openWebView,
                commonHeaders,
              ...
              seed =
                cheerio.load(driveres.data)(".btn-danger").attr("href") || "",
        ...
        return (console.log("katgetStream error: ", error), []);
      }
    },
  );
}, "getStream");
...
const __kandoProviderId = "katmovies";
